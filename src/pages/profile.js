...
export default function Profile() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-indigo-50 via-white to-purple-50">
      ...
      <div className="max-w-4xl px-4 pb-20 mx-auto">
        {/* Enhanced Profile Picture Section */}
        <motion.div
          initial={{ opacity: 0, y: 30, scale: 0.8 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 0.6, type: "spring", stiffness: 200 }}
          className="relative flex flex-col items-center mt-8 mb-12 lg:flex-row lg:items-center lg:space-x-8"
        >
          <div className="relative group">
            ...
            >
              <motion.img
                ...
                src={formData.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(formData.displayName || user?.email?.split('@')[0] || 'User')}&background=random&color=fff&size=200&bold=true`}
                alt="Profile"
                className="object-cover w-full h-full"
                onError={(e) => {
                  e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(formData.displayName || user?.email?.split('@')[0] || 'User')}&background=6366f1&color=fff&size=200&bold=true`;
                }}
              />
          ...
          <motion.div
            ...
            className="flex-1 mt-6 text-center lg:mt-0 lg:text-left"
          >
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
              ...
              <motion.div
                ...
                className="flex mt-4 space-x-3 lg:mt-0"
              >
                {isEditing ? (
                  <>
                    <motion.button
                      ...
                      className="flex items-center px-4 py-2 text-sm font-semibold text-gray-700 transition-all duration-300 border border-gray-200 rounded-lg shadow-lg bg-white/80 backdrop-blur-sm hover:bg-white hover:shadow-xl"
                    >
                    ...
                    <motion.button
                      ...
                      disabled={saving}
                      className={`flex items-center px-4 py-2 text-sm bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-lg font-semibold hover:from-indigo-600 hover:to-purple-600 transition-all duration-300 shadow-lg hover:shadow-xl ${saving ? 'opacity-75 cursor-not-allowed' : ''}`}
                    >
                      {saving ? (
                        <>
                          <motion.svg 
                  ...
                  <>
                    <motion.button
                      ...
                      className="flex items-center px-4 py-2 text-sm font-semibold text-white transition-all duration-300 rounded-lg shadow-lg bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 hover:shadow-xl"
                    >
                    ...
                    <motion.button
                      ...
                      disabled={loggingOut}
                      className={`flex items-center px-4 py-2 text-sm bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-lg font-semibold hover:from-red-600 hover:to-pink-600 transition-all duration-300 shadow-lg hover:shadow-xl ${loggingOut ? 'opacity-75 cursor-not-allowed' : ''}`}
                    >
                      {loggingOut ? (
                        <>
                          <motion.svg 
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <div className="relative p-6 border border-gray-100/50 rounded-2xl bg-white/60">
              <motion.h2 
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 0.3 }}
                className="flex items-center mb-6 text-lg font-bold text-gray-800 group"
              >
                <motion.div
              ...
              <div className="space-y-4">
                ...
                >
                  <label className="flex items-center mb-2 text-sm font-semibold tracking-wider text-gray-600 uppercase">
                    <FiUser className="mr-2 text-indigo-500" size={12} />
                    Display Name
                  </label>
                  {isEditing ? (
                    <motion.input
                      ...
                      className="w-full px-3 py-2 text-gray-800 transition-all duration-300 border-2 border-gray-200 rounded-lg shadow-sm bg-gray-50/50 focus:outline-none focus:border-indigo-500 focus:bg-white"
                      placeholder="Enter your name"
                    />
                  ) : (
                    <motion.p 
                      whileHover={{ x: 5 }}
                      className="px-3 py-2 font-medium text-gray-800 transition-all duration-300 border border-gray-100 rounded-lg bg-gray-50/30 group-hover:bg-gray-50/50"
                    >
                ...
                >
                  <label className="flex items-center mb-2 text-sm font-semibold tracking-wider text-gray-600 uppercase">
                    <FiStar className="mr-2 text-indigo-500" size={12} />
                    Status Message
                  </label>
                  {isEditing ? (
                    <motion.textarea
                      ...
                      className="w-full px-3 py-2 text-gray-800 transition-all duration-300 border-2 border-gray-200 rounded-lg shadow-sm resize-none bg-gray-50/50 focus:outline-none focus:border-indigo-500 focus:bg-white"
                      rows={2}
                    ...
                    <motion.p 
                      whileHover={{ x: 5 }}
                      className="px-3 py-2 text-gray-800 bg-gray-50/30 rounded-lg font-medium border border-gray-100 group-hover:bg-gray-50/50 transition-all duration-300 min-h-[2.5rem] flex items-center"
                    >
                      {formData.status}
                    </motion.p>
                  )}
            ...
            <div className="relative p-6 border border-gray-100/50 rounded-2xl bg-white/60">
              ...
              <div className="space-y-4">
                ...
                >
                  <label className="flex items-center mb-2 text-sm font-semibold tracking-wider text-gray-600 uppercase">
                    <FiMail className="mr-2 text-green-500" size={12} />
                    Email Address
                  </label>
                  <motion.div 
                    whileHover={{ x: 5 }}
                    className="flex items-center px-3 py-2 transition-all duration-300 border border-gray-100 rounded-lg bg-gray-50/30 group-hover:bg-gray-50/50"
                  >
                ...
                >
                  <label className="flex items-center mb-2 text-sm font-semibold tracking-wider text-gray-600 uppercase">
                    <div className="w-2 h-2 mr-2 bg-indigo-500 rounded-full" />
                    Member Since
                  </label>
                  <motion.p 
                    whileHover={{ x: 5 }}
